...
Pour faire ce TP, votre enseignant a créé un dépôt de code personnel pour chaque étudiant. \
Ce dépôt vous est propre et n'est accessible que par vous. Vous devez le récupérer (le cloner).

- Utilisez ce dépôt tout au long du TP; ne créez pas de nouveau projet ni de nouveau dépôt.
- Effectuez vos commits régulièrement dans ce dépôt (voir la consigne sur les commits ci-dessous).
:::
...
- Votre dépôt contient déjà tous les fichiers nécessaires à la réalisation de votre TP :
```
tp1/
├── .gitignore        # ce fichier vous est fourni et sert à exclure les fichiers à ne pas versionner
...
```
Les fichiers `racine.py` et `performance.py` sont ceux que vous devrez compléter. \
Votre dépôt de code ne doit contenir aucun autre fichier que les 4 fichiers fournis.
:::
...
Vous devez exécuter fréquemment le script `autocorrection.py`.
...
- Ajoutez, à vos deux fonctions, un paramètre `degre` correspondant à l’*indice* de la racine (2 pour une racine carrée, 3 pour une racine cubique, etc.).
...
- **Nombres négatifs**
    - Le calcul de la racine d'un nombre négatif doit être possible **uniquement si le degré est impair** (ex. : la racine cubique de -27 est -3).
    - Dans tous les autres cas, une exception doit être levée.
- **Validation du degré**
    - La valeur du paramètre `degre` ne doit pas être inférieure à 1.
