...
<Tabs queryString="onglet">
    ...
    <TabItem value="intro" label="Fonction" default>
        ...
        <Feedback surveyId="Rencontre #3 - Fonction" />
    </TabItem>
    ...
    <TabItem value="parametres" label="Paramètres">
        ...
        <Feedback surveyId="Rencontre #3 - Paramètres" />
    </TabItem>
    ...
    <TabItem value="retour" label="Retour">
        ...
        <Feedback surveyId="Rencontre #3 - Retour" />
    </TabItem>
    ...
    <TabItem value="portee" label="Portée">
        ...
        **Elle peut être utilisée dans tout le programme**, y compris à l’intérieur des fonctions,
        ...
        <Feedback surveyId="Rencontre #3 - Portée" />
    </TabItem>
    ...
    <TabItem value="DRY" label="DRY">
        ...
        <Feedback surveyId="Rencontre #3 - DRY" />
    </TabItem>
    ...
    <TabItem value="comments" label="Commentaires">
        ...
        <Feedback surveyId="Rencontre #3 - Commentaires" />
    </TabItem>
    ...
    <TabItem value="pile" label="Pile d'appels">
        ...
        <Feedback surveyId="Rencontre #3 - Pile d" />
    </TabItem>
    ...
    <TabItem value="Activités" label="Activités">
        ...
        <Tabs>
	        ...
	        <TabItem value="2" label=" 🏫 Activités à faire en classe">
                ...
                3) Écrivez une troisième fonction `aire_totale_prisme(longueur, largeur, hauteur)` qui calcule et **retourne** l’aire totale des 6 faces du prisme (la surface à peindre, par exemple).

                👉 Un prisme rectangulaire possède **3 paires de faces identiques** : deux faces $longueur \times largeur$, deux faces $longueur \times hauteur$ et deux faces $largeur \times hauteur$. \
                Votre fonction doit **appeler `aire_rectangle` trois fois** (une par paire de faces) avec les bonnes dimensions, additionner les 3 aires obtenues, puis multiplier le total par 2. \
                ⚠️ À vous de réfléchir à **quelles dimensions** transmettre à chaque appel : ce n’est pas une recopie de formule, c’est une décomposition à raisonner.

                4) Testez vos fonctions en affichant :
                - l’aire d’un rectangle de $5 \times 3$
                - le volume d’un prisme de $5 \times 3 \times 4$
                - l’aire totale d’un prisme de $5 \times 3 \times 4$ (réponse attendue : $94$)

                *Objectif :* pratiquer la décomposition d’un problème en plusieurs fonctions réutilisables, en réfléchissant aux arguments à transmettre à chaque appel.

                :::note Notions visées
                Définition de fonctions, `return`, appel d'une fonction depuis une autre (réutilisation), appels multiples d'une même fonction, décomposition d'un problème
                :::
                ...
                Dans cet exercice, vous allez séparer votre code en **deux fichiers** : un **module**, qui contient une fonction réutilisable, et un **script**, qui importe ce module pour utiliser la fonction.

                📁 Créez les deux fichiers suivants, **dans le même dossier** :

                | Fichier                    | Rôle                                                   |
                | -------------------------- | ------------------------------------------------------ |
                | `sciences_nature.py`       | Le **module** : il contient la fonction de calcul.     |
                | `r03_energie_cinetique.py` | Le **script** : il importe le module et affiche les résultats. |

                Au besoin, référez-vous à la [recette sur les modules](/recettes/creer-module).

                **Étape 1 : le module `sciences_nature.py`**

                Écrivez une fonction `energie_cinetique(masse, vitesse)` qui **retourne** l'énergie cinétique $E$ calculée selon la formule :

                $$
                E = \frac{m \times v^2}{2}
                $$

                où $m$ est la masse (en kg), $v$ est la vitesse (en m/s) et $E$ est l'énergie (en joules, J).

                :::tip
                La fonction ne doit rien afficher : elle utilise `return` pour renvoyer le résultat. C'est le script qui s'occupe de l'affichage.
                :::

                **Étape 2 : le script `r03_energie_cinetique.py`**

                1. Importez le module `sciences_nature`.
                2. Appelez la fonction `energie_cinetique` pour chacun des cas suivants.
                3. Affichez chaque résultat avec une phrase claire.

                | Objet    | Masse (kg) | Vitesse (m/s) | Énergie attendue (J) |
                | -------- | ---------- | ------------- | -------------------- |
                | Ballon   | 0.5        | 10            | 25.0                 |
                | Cycliste | 80         | 8             | 2560.0               |
                | Voiture  | 1500       | 25            | 468750.0             |

                Exemple d'affichage attendu :

                ```
                Énergie cinétique du ballon : 25.0 J
                Énergie cinétique du cycliste : 2560.0 J
                Énergie cinétique de la voiture : 468750.0 J
                ```

                Utilisez le tableau pour vérifier que votre fonction donne les bons résultats.

                :::note Notions visées
                Modules (créer et importer), définition de fonction, paramètres, `return`, opérateurs arithmétiques (`**`, `/`)
                :::
                ...
                Dans cet exercice, vous allez **enrichir le module** créé à l'exercice précédent en y ajoutant une deuxième fonction, puis l'utiliser dans un nouveau script.

                📁 Vous travaillerez avec les deux fichiers suivants, **dans le même dossier** :

                | Fichier                         | Rôle                                                              |
                | ------------------------------- | ----------------------------------------------------------------- |
                | `sciences_nature.py`            | Le **module** de l'exercice précédent, que vous allez compléter.  |
                | `r03_force_gravitationnelle.py` | Le **nouveau script** : il importe le module et affiche les résultats. |

                **Étape 1 : compléter le module `sciences_nature.py`**

                1. Au début du module, définissez une constante `G` pour la constante gravitationnelle :
                   $G = 6.674 \times 10^{-11}$ N·m²/kg². En Python, cette valeur s'écrit `6.674e-11`.
                2. Sous la fonction `energie_cinetique` (ne la supprimez pas!), ajoutez une fonction `force_gravitationnelle(m1, m2, r)` qui **retourne** la force d'attraction entre deux objets, selon la loi de la gravitation universelle de Newton :

                $$
                F = \frac{G \times m_1 \times m_2}{r^2}
                $$

                où $m_1$ et $m_2$ sont les masses des deux objets (en kg), $r$ est la distance entre leurs centres (en m) et $F$ est la force (en newtons, N).

                :::tip Le poids, un cas particulier
                Le **poids** d'un objet est la force gravitationnelle qu'exerce une planète sur lui. Pour le calculer, on utilise la masse de la planète pour $m_1$, la masse de l'objet pour $m_2$, et le rayon de la planète pour $r$.
                :::

                **Étape 2 : le script `r03_force_gravitationnelle.py`**

                1. Importez le module `sciences_nature`.
                2. Appelez la fonction `force_gravitationnelle` pour chacun des cas suivants.
                3. Affichez chaque résultat avec une phrase claire.

                | Situation                           | $m_1$ (kg)            | $m_2$ (kg)            | $r$ (m)               | Force attendue (N)         |
                | ----------------------------------- | --------------------- | --------------------- | --------------------- | -------------------------- |
                | Poids d'une personne sur la Terre   | $5.972 \times 10^{24}$ | 70                    | $6.371 \times 10^{6}$ | ≈ 687.4                    |
                | Poids d'une personne sur la Lune    | $7.348 \times 10^{22}$ | 70                    | $1.737 \times 10^{6}$ | ≈ 113.8                    |
                | Attraction entre la Terre et la Lune | $5.972 \times 10^{24}$ | $7.348 \times 10^{22}$ | $3.844 \times 10^{8}$ | ≈ $1.982 \times 10^{20}$  |

                Les valeurs attendues sont arrondies : il est normal que votre programme affiche plus de décimales.

                :::tip Pour un affichage plus lisible (facultatif)
                Dans une f-string, `{force:.1f}` affiche un nombre avec une décimale, et `{force:.3e}` l'affiche en notation scientifique avec trois décimales (par exemple `1.982e+20`).
                :::

                :::note Notions visées
                Modules (compléter et importer), constantes, définition de fonction à plusieurs paramètres, `return`, opérateurs arithmétiques (`**`), notation scientifique en Python (`e`)
                :::
                ...
                **Beaucoup d'instructions** servant à afficher des données **sont très similaires** et sont utilisées dans le **même ordre**. \
                ...
                Ajoutez une **ligne supplémentaire** qui affiche la **densité moyenne** des 3 échantillons. \
                Pas besoin de recalculer quoi que ce soit : placez les 3 densités déjà obtenues dans une **liste**, \
                puis calculez la moyenne en divisant la **somme des éléments de la liste** par le **nombre d'éléments de la liste**.

                ✅ Sortie attendue (avec la ligne supplémentaire)
                ```
                Densité 1 : 1607.14 kg/m3
                Densité 2 : 1788.08 kg/m3
                Densité 3 : 814.16 kg/m3
                Densité moyenne : 1403.13 kg/m3
                ```

                :::note Notions visées
                Refactorisation, listes parallèles, accès par index, définition de fonction, `return`, `round()`, `sum()` / `len()` (pour aller plus loin)
                :::
                ...
                **Exemple attendu (pour $m = 1200$, $v = 25$, $r = 50$) :**
                ...
                - [Conversion Celsius → Fahrenheit](https://youtu.be/rZXaiUvlkhw)
                - [Aire d'un rectangle](https://youtu.be/-7xRSkuWON0)
                - [Réutilisation de fonctions](https://youtu.be/nSwt71ZJ9EA)
                - [Énergie cinétique](https://youtu.be/QX13BfHN_AA)
                - [Force gravitationnelle](https://youtu.be/OQIA5eDnt_k)
                - [Bonjour](https://youtu.be/II-Lbfh5INM)
                - [Refactorisation du code - Umbrella Corporation](https://youtu.be/PFPNMIG5_hg)
                - [Refactorisation du code - Calcul de densité](https://youtu.be/iT-WonFZ_ww)
                - Une question de nanosecondes — [solutionnaire.pdf](/pdf/r03_nanosecondes_solutionnaire.pdf)

                ✏️ Solution des **exercices de compréhension** :
                - [Force centripète](https://youtu.be/SnAeXeza2us)
    ...
    <TabItem value="param-fonction" label="*Fonction en paramètre*">
        ...
        *Vous pouvez utiliser ou non cette notion pour réduire la quantité de code de la section **Comparaison des performances** du **TP1**.*
        ...
        <Feedback surveyId="Rencontre #3 - Fonction en paramètre" />
    </TabItem>
    ...
    <TabItem value="recusivite" label="*Récursivité*">
        ...
        <Feedback surveyId="Rencontre #3 - Récursivité" />
    </TabItem>
