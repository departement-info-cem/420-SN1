...
<Tabs queryString="onglet">
    ...
    <TabItem value="DRY" label="DRY">
        ...
        Prenons un exemple en physique spatiale où l'on calcule et affiche le poids d'un spationaute de 75 kg sur différents astres (rappel : le poids est une force en Newtons calculée par la formule $P = m \times g$).
        ...
        print(f"Sur l'astre Terre, le poids est de {poids_terre} Newtons.")

        # Calcul et affichage pour Mars
        poids_mars = masse_spationaute * 3.71
        print(f"Sur l'astre Mars, le poids est de {poids_mars} Newtons.")

        # Calcul et affichage pour la Lune
        poids_lune = masse_spationaute * 1.62
        print(f"Sur l'astre Lune, le poids est de {poids_lune} Newtons.")
        ```
        ...
        :::tip Étape 1 : Identifier les éléments qui changent
        ...
        ::::info Étape 2 : Extraire la fonction de calcul (avec retour)
        ...
        :::note Résultat intermédiaire : Intégration du calcul

        À cette étape, nous avons centralisé la logique mathématique. Le script principal appelle maintenant notre fonction `calculer_poids` au lieu de réécrire la formule à chaque fois.
        ...
        print(f"Sur l'astre Terre, le poids est de {p_terre} Newtons.")

        p_mars = calculer_poids(masse_spationaute, 3.71)
        print(f"Sur l'astre Mars, le poids est de {p_mars} Newtons.")

        p_lune = calculer_poids(masse_spationaute, 1.62)
        print(f"Sur l'astre Lune, le poids est de {p_lune} Newtons.")
        ```

        👉 **Observation** : Le calcul est maintenant sécurisé dans une fonction (si on veut changer la précision, on ne le fait qu'à un seul endroit). Cependant, on remarque que les instructions `print` sont encore très répétitives. C'est ce que nous allons régler à l'étape suivante.
        :::

        ::::info Étape 3 : Extraire la fonction d'affichage (sans retour)

        Maintenant, occupons-nous de la phrase affichée.
        Nous allons créer une deuxième fonction qui gère le texte, et qui appellera notre première fonction pour obtenir la valeur de la force gravitationnelle!
        ...
        def afficher_poids_astre(nom_astre, m, g):
            ...
            print(f"Sur l'astre {nom_astre}, le poids est de {poids_calcule} Newtons.")
        ```
        ::::

        :::note Le code final simplifié
        ...
        👉 Si demain on veut changer la phrase affichée, ou ajouter un arrondi au calcul, on n'aura qu'à modifier **un seul endroit** dans le code au lieu de trois!
        :::
    ...
    <TabItem value="param-fonction" label="*Fonctions en paramètres*">
        ...
        *Vous pouvez utilisez ou non cette notion pour réduire la quantité de code de la section **Comparaison des performances** du **TP1**.*
        :::
        ...
        ::::info Étape 2 : Créer une fonction recevant en paramètre le nom d'une fonction

        C'est ici que la magie opère. On crée une fonction qui accepte une valeur ET une **autre fonction** comme paramètre.

        ```python showLineNumbers
        # Le paramètre 'transformation' recevra le NOM d'une fonction (voir l'appel de cette fonction à l'étape #3)
        ...
        :::tip Étape 3 : Appeler la fonction en lui passant le nom d'une fonction en paramètre

        Lors de l'appel, on passe le nom de la fonction de conversion **sans les parenthèses**. \
