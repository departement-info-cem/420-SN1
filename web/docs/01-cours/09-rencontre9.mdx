...
<Tabs queryString="onglet">
    ...
    <TabItem value="foreach" label="For (séquence)">
        ...
        Voici un exemple avec une liste de héros :
        ```python showLineNumbers
        ...
        À chaque tour de boucle, la variable `hero` prend automatiquement la valeur de l'élément suivant dans la liste. \
        Son nom est libre : on aurait pu l'appeler `h`, `personnage`, ou n'importe quoi d'autre. C'est nous qui choisissons.

        👉 Exécutez ce code en mode débogage. Mettez un point d'arrêt sur la ligne 2 et avancez une ligne à la fois pour suivre l'évolution de la variable `hero`.

        :::

        ---

        :::note Équivalence avec `while` et `range()` : ce que tu connaissais déjà
        ...
        <Row>
            ...
            <Column>
                ...
                for i in range(len(heros)):
                    print("Bonjour", heros[i])
                ```
            </Column>
        </Row>

        La version `for hero in heros` est plus concise et plus facile à lire, surtout quand on n'a pas besoin de l'index.
        :::

        ---

        :::tip Quand utiliser quelle boucle?
        - `for hero in heros` : quand on veut traiter chaque élément et qu'on n'a **pas besoin de l'index**.
        - `for i in range(len(heros))` : quand on a **besoin de l'index** pour modifier la liste ou comparer des positions.
        ...
        Voici un exemple qui crée une liste de messages de bienvenue à partir de la liste de héros :

        ```python showLineNumbers
        heros = ["Mario", "Link", "Kirby", "Sonic"]
        messages = []
        for hero in heros:
            messages.append(f"Bienvenue, {hero}!")  # Ajoute un message à la liste
        print(messages)
        ```
        ```
        ['Bienvenue, Mario!', 'Bienvenue, Link!', 'Bienvenue, Kirby!', 'Bienvenue, Sonic!']
        ```
        ...
        La syntaxe `for hero in heros` est pratique, mais elle ne permet **pas** de parcourir deux listes en parallèle, puisqu'on n'a pas accès à l'index. \
        Dans ce cas, il faut revenir à `for i in range(len(...))` :

        ```python showLineNumbers
        # ❌ Impossible avec for hero in heros — pas d'accès à l'index
        heros  = ["Mario", "Link", "Kirby", "Sonic"]
        scores = [4200, 3800, 5100, 2900]

        # ✅ Utiliser range() pour accéder aux deux listes via l'index
        for i in range(len(heros)):
            print(f"{heros[i]} a un score de {scores[i]}")
        ```
        ```
        Mario a un score de 4200
        Link a un score de 3800
        Kirby a un score de 5100
        Sonic a un score de 2900
        ```
        ...
        👉 Exécutez-le en mode débogage pour suivre l'évolution des variables `index` et `hero` à chaque itération.
        :::
    ...
    <TabItem value="dict" label="Dictionnaire (dict)">
        ...
        mon_dict = {"nom": "Alice", "âge": 30, "ville": "Montréal"}
        del mon_dict["ville"]
        print(mon_dict)  # Résultat : {'nom': 'Alice', 'âge': 30}
        ```

        :::danger Clé inexistante
        Si la clé n'existe pas dans le dictionnaire, `del` lève une erreur `KeyError` :

        ```python
        del mon_dict["pays"]  # KeyError: 'pays'
        ```

        Pour éviter cette erreur, on peut d'abord vérifier que la clé est présente :
