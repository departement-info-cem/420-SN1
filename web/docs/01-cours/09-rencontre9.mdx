...
<Tabs queryString="onglet">
    ...
    <TabItem value="foreach" label="For (séquence)">
        ...
        À la rencontre 5, tu as parcouru des listes avec `for i in range(len(liste))` et `liste[i]`. \
        ...
        :::note Équivalence avec une boucle `for` et `range(len)` : ce que tu connaissais déjà
        ...
        👉 Exécutez-le en mode débogage pour suivre l'évolution de la liste `messages` à chaque itération.
        :::
    ...
    <TabItem value="listcomp" label="Compréhension de liste">
        ...
        👉 Si tu trouves ce qui suit difficile à suivre, c'est tout à fait normal. Cette notion dépasse les attentes du cours. Tu peux passer directement à l'onglet suivant sans te culpabiliser.
    ...
    <TabItem value="set" label="Ensemble (set)">
        ...
        **Différence `-`** : les éléments qui sont dans le premier ensemble mais **pas** dans le second. Ce que le premier a en exclusivité.
        ...
        Si l'ordre est important pour toi, utilise une **liste** plutôt qu'un ensemble.
    ...
    <TabItem value="matplotlib" label="Matplotlib">
        ...
        :::tip 🔍 Utilise le débogueur en premier
        Avant de chercher l'erreur à la main, **utilise le débogueur PyCharm**. \
        Place un point d'arrêt avant `plt.plot()` et inspecte tes listes dans le panneau des variables :
        - vérifie leur longueur avec `len()`
        - vérifie leur type avec `type()`
        - vérifie leurs valeurs

        Le débogueur te montre exactement ce que Matplotlib reçoit comme données.
        :::

        :::danger 1. `ValueError: x and y must have same first dimension`
        Les listes `x` et `y` n'ont pas le même nombre d'éléments. Vérifie que leurs longueurs sont identiques.
        ...
        Tu as oublié `plt.show()`, ou tu as appelé `plt.show()` avant de tracer les données. `plt.show()` doit toujours venir **en dernier**.
