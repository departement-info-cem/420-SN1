...
<Tabs className="tabs-centered-left" queryString="onglet">
    ...
    <TabItem value="pandas" label="🐼 Pandas DataFrame">
        ...
        :::note 🔍 Visualiser un fichier CSV directement dans PyCharm
        ...
        :::warning ⚠️ Limitation du nombre de lignes affichées
    ...
    <TabItem value="pandasselectionner" label="Sélectionner Series Parcourir">
        ...
        On peut aussi sélectionner une **seule valeur** à l’intersection d’une ligne et d’une colonne spécifiques :

        ```python
        # Sélectionner la ligne 3 (donne une Series) puis la colonne "total_bill" (donne une valeur)
        valeur_cellule = df.iloc[3]["total_bill"]
        ...
        :::tip convertir une Series en liste Python

        Parfois, on peut vouloir convertir une *Series* en une simple liste Python. \
        Par exemple, pour convertir la colonne `day` du *DataFrame* `df` en une liste de jours, on peut faire :
        ```python
        liste_jours = df["day"].tolist()
        print(liste_jours)
        ```
        Cela produira une liste Python contenant les valeurs de la colonne `day` :
        ```
        ['Sun', 'Sun', ..., 'Sat', 'Sat']
        ```
