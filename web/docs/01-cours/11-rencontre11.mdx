...
<Tabs queryString="onglet">
    ...
    <TabItem value="data" label="Intro" default>
        ...
        À la rencontre 10, tu as appris à utiliser NumPy pour calculer sur des tableaux de nombres. NumPy est puissant, mais il a une limite : il travaille sur des données **d’un seul type** (tous des nombres, ou tous des textes). Dans la réalité, un jeu de données scientifiques contient des colonnes de types différents : des mesures numériques, des dates, des catégories, des noms. Pour manipuler ce genre de données, NumPy seul devient vite insuffisant :
        ...
        C’est là qu’intervient 🐼 **Pandas** : il regroupe toutes ces colonnes dans un seul objet, le **DataFrame**, et offre des outils pour les manipuler ensemble facilement.
        ...
        - 🔢 [**NumPy**](/cours/rencontre10?onglet=numpy) : calcul scientifique sur des tableaux (**vu à la rencontre 10**)
    ...
    <TabItem value="pandas" label="Explorer">
        ...
        Dans un DataFrame, chaque colonne a un type de données (`Dtype`). Tu as déjà rencontré ce concept en NumPy avec `tableau.dtype` : c'est exactement la même notion, appliquée à une colonne. Voici les types les plus courants et leur équivalent Python :
    ...
    <TabItem value="pandasfiltrer" label="Filtrer / Trier">
        ...
        :::note 💡 Tu connais déjà ce principe grâce à NumPy
    ...
    <TabItem value="pandasselectionner" label="Sélectionner">
        ...
        **Une seule colonne → DataFrame** (doubles crochets)
        ...
        Sous le capot, chaque colonne Pandas est stockée comme un tableau NumPy. On peut y accéder directement avec `.values`, ce qui retourne un `ndarray` prêt à utiliser avec toutes les fonctions NumPy que tu connais :
        ...
        C'est utile quand tu veux appliquer des opérations NumPy (`np.clip`, `np.where`, `np.mean`, etc.) sur des données provenant d'un DataFrame.
    ...
    <TabItem value="pandastransformer" label="Transformer">
        ...
        :::note 💡 La vectorisation que tu connais déjà de NumPy
        En NumPy, tu as appris que `tableau * 2` applique l'opération à tous les éléments sans boucle. Pandas utilise exactement le même principe : `df["col"] * 2` applique la multiplication à toute la colonne en une seule instruction. Tu n'as pas besoin de boucle `for`, pour la même raison qu'en NumPy.
        :::
        ...
        **Comparaison avec une liste de valeurs (`.isin()`) :**
        ...
        Tu connais déjà `np.where` de la [rencontre 10](/cours/rencontre10?onglet=numpy-conditions). En Pandas, on peut faire exactement la même opération avec `np.where` directement sur une colonne, ou avec `apply` :

        ```python
        import numpy as np

        # Avec np.where (que tu connais déjà de NumPy)
    ...
    <TabItem value="pandasgrouper" label="Statistiques">
        ...
        Tu connais déjà `np.mean()`, `np.min()`, `np.max()` de la [rencontre 10](/cours/rencontre10?onglet=numpy-vectorise). En Pandas, ces mêmes opérations s'appellent directement sur la colonne : `df["col"].mean()` plutôt que `np.mean(tableau)`. Le résultat est identique, seule la syntaxe change.
        :::
    ...
    <TabItem value="pandasvisualiser" label="Visualiser">
        ...
        Si tu veux te rafraîchir la mémoire sur Matplotlib, consulte l'[onglet Matplotlib de la rencontre 9](/cours/rencontre9?onglet=matplotlib). \
        Pour aller plus loin, consulte aussi la [documentation officielle de Matplotlib](https://matplotlib.org/stable/contents.html).
