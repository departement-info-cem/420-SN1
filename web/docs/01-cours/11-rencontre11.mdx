...
<Tabs queryString="onglet">
    ...
    <TabItem value="pandasselectionner" label="Sélectionner / Parcourir">
        ...
        Par exemple, pour sélectionner certaines colones spécifiques, on utilise une liste de noms de colonnes :
        ```python
        ...
        On peut aussi le faire directement sans créer de variable intermédiaire pour la liste de noms (mais dans ce cas il ne faut pas oublier les doubles crochets `[[...]]`) :
        ```python
        ...
        On peut aussi sélectionner des lignes spécifiques à l'aide de différentes méthodes, notamment `head()`, `iloc[]` :
        ...
        df_plage_lignes = df.iloc[3:10] # Lignes 3 à 9
        ...
        Une *Series* est une structure de données à une dimension, similaire à une liste, qui contient des données d'une seule colonne ou d'une seule ligne.

        ```python
        # Créer une Series avec seulement les données de la colonne "total_bill"
        serie_total_bill = df["total_bill"] # pas de double [[...]], donc pas une liste, mais un seul nom de colonne
        print(serie_total_bill)

        # Créer une Series avec uniquement les données de la ligne 4
        serie_une_ligne = df.iloc[3]  # pas d'intervalle de lignes, mais un seul index de ligne (4ème ligne - index 3)
        ...
        **💡 Observations importantes**
        ...
        C'est un type de données à *1 dimension* (comme les listes de données). \
        2 cas possibles :
        ...
        Par exemple, les opérations de filtrage et triage vues précédemment étaient pour les *DataFrames*. \
        En cas de doute, si vous souhaitez valider le type de l'objet contenu dans votre variable, utilisez la fonction `type(...)`.

        :::

        :::tip 🔁 Parcourir ligne par ligne un DataFrame et une Series
        ...
        # Boucle pour parcourir les lignes de ce DataFrame
        for index, row in df.iterrows():
            print(f"Index : {index}, Nom : {row['Nom']}, Age : {row['Age']}")

        # Boucle pour parcourir une Series (la sélection d'une seule colonne d'une DataFrame génère une Series)
        for x in df["Nom"]:
            print(f"Nom : {x})
        ```
        ...
        Cette méthode permet de récupérer la valeur à l'intersection d'une ligne et d'une colonne spécifiées.
