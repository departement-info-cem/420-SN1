...
Voici comment Python évalue l’instruction #5, étape par étape :
    ...
    <TabItem value="imbrication" label="Imbrication">
        ...
        Il est tout à fait possible d'écrire une structure conditionnelle **à l'intérieur** d'une autre structure conditionnelle.\
        On appelle cela des structures **imbriquées** (*nested* en anglais).

        C'est utile quand une décision dépend d'une autre décision comme dans la vraie vie!
        :::
        ...
        if ph < 7 :
            ...
            if temperature_celsius > 60 :               # Condition imbriquée dans le else
                print("⚠️ Basique ET chaude : manipuler avec précaution.")
            else :
        ...
        👉 Notez comment le premier `if/elif/else` traite la luminosité, et qu'à l'intérieur de **chacun** de ses blocs, un second `if/elif/else` affine la classification selon la température.
        :::
        ...
        En général, **2 à 3 niveaux maximum** est une bonne règle de base.\
        ...
        | Les conditions sont **indépendantes** l'une de l'autre | Plusieurs `if` séparés |
