...
Voici comment Python évalue l’instruction #5, étape par étape :
    ...
    <TabItem value="comparaison" label="comparaison">
        ...
        Passable
        ```
        👉 Avec des `if` indépendants, Python évalue **chaque condition séparément**, de haut en bas, **sans jamais s'arrêter**. Comme les 3 conditions sont `True` pour `score = 95`, les **3 blocs sont exécutés**. Ce n'est presque certainement pas ce qu'on voulait! Il aurait été préférable que Python arrête ne tente pas d'exécuter les conditions des autres blocs dès qu'il trouve une condition donnant `True`.
        ...
        👉 Avec `if/elif/else`, dès qu'une condition est `True`, Python exécute le bloc correspondant et **saute immédiatement le reste** -
         les autres conditions ne sont **jamais évaluées**. Un seul bloc s'exécute, peu importe le nombre de `elif`. Python a exécuté les instructions `2`, `3` et `4` uniquement!
        :::
        ...
        **Exemple où plusieurs `if` séparés sont appropriés** :
        ...
        - **Ordre des conditions** :
            ...
            - les autres conditions sont ignorées.
        :::
