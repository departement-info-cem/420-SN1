...
<Tabs className="tabs-centered" queryString="onglet">
    ...
    <TabItem value="numpy" label="NumPy (intro)">
        ...
        :::tip 💡 Cette section en bref

        Jusqu’ici, pour travailler avec une liste de nombres, tu devais écrire des boucles `for` : une itération par élément, une opération à la fois. \
        ...
        :::info 🔢 Ce que tu vas apprendre dans les onglets NumPy
        ...
        NumPy n’est qu’une des bibliothèques que tu utiliseras tout au long de ta formation. \
        Pour ta culture générale, voici comment elle s’inscrit dans l’écosystème Python scientifique :

        - 🔢 **NumPy** : calcul sur des tableaux numériques - **c’est le sujet de ces onglets**
        - 🐼 **Pandas** : manipulation de données tabulaires - **vu à la rencontre 11**
        - 📊 **Matplotlib** : visualisation de données - **tu le connais déjà**
        - 🤖 **Scikit-learn** : apprentissage automatique - **aperçu dans à la rencontre 15**
        - 🧠 **Keras / TensorFlow** : apprentissage profond - **vue dans un cours ultérieur**
        ...
        NumPy n’est pas inclus avec Python. Tu dois l’installer avant de pouvoir l’utiliser. \
    ...
    <TabItem value="numpy-tableaux" label="Tableaux NumPy">
        ...
        En Python, une liste peut contenir n’importe quoi : des nombres, des textes, des listes, des objets mélangés. \
        ...
        NumPy tire parti de cette contrainte pour effectuer des calculs beaucoup plus rapidement, en traitant le tableau entier d’un seul bloc plutôt qu’élément par élément.
        ...
        Tu verras `import numpy as np` au début de tous les scripts qui utilisent NumPy. C’est une convention que tout le monde respecte. Utilise-la aussi.
        ...
        **Avec des séquences de valeurs :**
        ```python
        sequence   = np.arange(0, 10, 2)   # [0 2 4 6 8]                 entiers de 0 à 9 par pas de 2, fonctionne comme la fonction range()
        ...
        **`shape`** - la forme du tableau, sous forme de tuple :
        ...
        Dans ce cours, tu travailleras presque exclusivement avec des tableaux **1D**. Les tableaux 2D seront davantage utilisés dans les cours suivants (réseaux de neurones, traitement d'images).
        :::
        ...
        **`dtype`** - le type des données stockées :
        ...
        Si tu crées un tableau avec un entier et un décimal, NumPy choisit automatiquement le type le plus général :
        ...
        **`size`** - le nombre total d’éléments :
        ...
        print(tableau[1:4])  # [20 30 40]  → du 2e au 4e inclus
        ```
    ...
    <TabItem value="numpy-vectorise" label="Calcul vectorisé">
        ...
        En Python ordinaire, pour doubler toutes les valeurs d’une liste, tu dois écrire une boucle :

        ```python
        # Avec une liste Python : une boucle obligatoire
        valeurs = [10, 20, 30, 40, 50]
        doubles = [v * 2 for v in valeurs]
        ```
        ...
        doubles = valeurs * 2   # [20 40 60 80 100]
        ```
        ...
        Sur de petites listes la différence de vitesse est imperceptible, mais sur des millions de valeurs, elle devient dramatique. Voici un exemple concret avec mesure du temps :
        ...
        On peut aussi combiner deux tableaux :
        ...
        Plutôt que de retourner la valeur maximale ou minimale, `argmax` et `argmin` retournent **l’indice** de cet élément dans le tableau. \
    ...
    <TabItem value="numpy-bornes" label="Bornes et correction">
        ...
        Utilise `np.clip(tableau, min, max)` quand tu connais à l’avance **deux bornes** (un minimum ET un maximum) et que tu veux garantir que toutes les valeurs restent dans cet intervalle en une seule instruction.
        ...
        :::info 🔼🔽 `np.maximum` et `np.minimum` - appliquer une borne à la fois

        Utilise `np.maximum` ou `np.minimum` quand tu veux appliquer **une seule borne à la fois** (un plancher OU un plafond), ou quand tu veux comparer deux tableaux position par position. Contrairement à `np.clip` qui applique les deux bornes en une seule instruction, `np.maximum` et `np.minimum` s'utilisent séparément, ce qui les rend plus flexibles.


        **Avec une constante :** \
        `np.maximum(tableau, valeur_max)` \
        `np.minimum(tableau, valeur_min)`

        Un cas concret : un capteur de température peut retourner des valeurs légèrement négatives à cause du bruit de mesure. \
        On veut s'assurer que toutes les températures sont au moins à 0°C, et qu'aucune ne dépasse 100°C :

        ```python
        temperatures = np.array([-1.2, 15.4, 98.7, 102.3, 0.0, 36.6])

        sans_negatifs = np.maximum(temperatures, 0)      # [-1.2 → 0, les autres inchangés]
        sous_ebullition = np.minimum(sans_negatifs, 100) # [102.3 → 100, les autres inchangés]
        ```

        **Avec deux tableaux :** \
        `np.maximum(tableau_a, tableau_b)` \
        `np.minimum(tableau_a, tableau_b)`

        ```python
        a = np.array([1, 8, 3, 6])
        b = np.array([5, 2, 7, 4])

        plus_grands = np.maximum(a, b)  # [5 8 7 6]  prend le plus grand à chaque position
        plus_petits = np.minimum(a, b)  # [1 2 3 4]  prend le plus petit à chaque position
        ```
