...
<Tabs queryString="onglet">
    ...
    <TabItem value="texte" label="Fichier texte">
        ...
        :::info Mode ajout : `"a"` (*append*)

        Le mode `"w"` **écrase** le contenu existant à chaque ouverture. \
        Si on veut plutôt **ajouter** des données à la fin d'un fichier sans effacer ce qui s'y trouve déjà, on utilise le mode `"a"`.
