...
plt.xticks(
    ...
    fontsize = 12       # taille de la police (défaut : taille globale du thème)
)
...
```python
plt.legend(
    loc          = 'lower right',  # position (défaut : 'best' — choisie automatiquement)
                                   # autres : 'upper right' | 'lower left' | 'center' | etc.
    title        = "Fonction",     # titre de la légende (défaut : None)
    title_fontsize = 12,           # taille du titre (défaut : taille globale du thème)
...
```python
plt.scatter(x, y, alpha=0.5, s=100)  # alpha : opacité (défaut : 1.0) | s : taille des points en pixels² (défaut : 20)
plt.show()
```
